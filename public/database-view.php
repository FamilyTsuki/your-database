...
// Get objects
$objets_res = $conn->query("SELECT objets.*, cat.nom AS nom_categorie, cat.parent_id, parent_cat.nom AS parent_nom FROM objets LEFT JOIN categories AS cat ON objets.id_categorie = cat.id LEFT JOIN categories AS parent_cat ON cat.parent_id = parent_cat.id WHERE objets.database_id = '$database_id' ORDER BY objets.id DESC");
$objets = [];
