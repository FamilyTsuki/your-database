# Script d'initialisation de la base de données pour Mon Inventaire (Windows)

Write-Host "📦 Mon Inventaire - Initialisation" -ForegroundColor Green
...
$sqlContent | Set-Content $tempSqlFile

# Exécuter le script SQL
try {
    if ($dbPass) {
        mysql -h $dbHost -u $dbUser -p$dbPass $dbName -e "source $tempSqlFile"
    } else {
        mysql -h $dbHost -u $dbUser $dbName -e "source $tempSqlFile"
    }
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Base de données initialisée avec succès!" -ForegroundColor Green
        Write-Host ""
        Write-Host "Vous pouvez maintenant accéder à l'application:" -ForegroundColor Green
        Write-Host "📍 http://localhost/your-database/public/" -ForegroundColor Cyan
    } else {
